...
var rightHalf = function () {
  ...
  if (window) {
    var screenFrame = window.screen().flippedFrame()
    window.setFrame({
      x: parseInt(screenFrame.width / 2),
      y: screenFrame.y,
      width: parseInt(screenFrame.width / 2),
      height: screenFrame.height,
    })
...
resizeMode.addSubShortcut('a', [], half)
resizeMode.addSubShortcut('d', [], rightHalf)

moveMode.addSubShortcut('m', [], maximise)
moveMode.addSubShortcut('=', [], center)
moveMode.addSubShortcut('a', [], half)
moveMode.addSubShortcut('d', [], rightHalf)
